...
drgbnk_db <- 
  readRDS('data_raw/pert_overlap_cmap_drugbank_stitch.rds') %>% 
...
load("gene_signatures_03-18-21.RData")
...
unweighted_random_res_sub[6:7] <- 
  # map(1:nrow(gene_sets), function(i){
  map(6:7, function(i){ # re-running 2 sigs because t
    x <- gene_sets[i,]
...
```{r}
load("unweighted_cmap_res_db_sub_03-22-21.RData")
...
load("../thesisAnalysis/gene_signatures_03-18-21.RData")


cmap_res <- read_rds("../thesisAnalysis/unweighted_res_sub_w_p-values_03-22-21.rds")
...
sig_idx_map <- read_csv("../thesisAnalysis/Data_file_S3_Gene_signatures.csv") %>% 
  select(1:3)
...
drg_db <- read_rds("data-raw/GSE92742_perts_in_DrugBank_03-30-21.rds") 
...
```{r}
load('data-raw/Fen_drugbank_and_stitch_data.RData')
...
db_cmap_matches <- 
  lapply(cols_to_keep, function(x){
    ...
    # inner_join(trt_cps, db_sub, na_matches = "never")
}) %>% do.call(rbind, .)
# saveRDS(db_cmap_matches, file = 'GSE92742_perts_in_DrugBank.rds')
...
db_cmap_matches <- db_cmap_matches %>% 
  ...
  distinct
man_annots <- read_rds("manual_lincs_drugbank_drug_annots.rds")
...
sig_pth_res <- 
  read_rds("lv_on_gsva_msig_kegg_pathways_from_row_scaled_clusters.rds") %>% 
  filter(adj.P.Val < .001)
coef_nms <- c("PLI vs. N&S", "PF vs. N&S", "PF vs. PLI" ) %>% 
  setNames(.,unique(sig_pth_res$coefficient))
...
#TODO Refactor this bit 
pth_map <- 
  read_delim("fens_categories.txt", ";", col_names = FALSE) %>% 
  setNames(., c("fens_category", "disease_category"))

fen_pths <- left_join(fen_pths, pth_map)
fen_pths$fens_category <- paste0("C", fen_pths$fens_category)
# sig_pth_res <- left_join(sig_pth_res, fen_pths)

nafld_category <- split(fen_pths, fen_pths$id) %>% 
  map(function(x){
    tmp_cats <-  x %>% 
      pull(fens_category) %>% 
      paste0(., collapse = '|') %>% 
...
pth_refs <- 
  read_excel("/Users/daniellefever/Desktop/Lans_Taylor_Lab/NAFLD_MPS/presentations_and_meetings/update_presentations/210312/pth_refs.xlsx", sheet = 1)
...
# write_csv(split_tbls[[1]], "foo.csv")
...
old_gene_sigs <- read_csv("Data_file_S3_Gene_signatures.csv")

sig_ids <- old_gene_sigs[,1:3]
...
new_gene_sig <- 
  unweighted_gene_sig %>% 
  select(-coefficient, -fens_category, -n_upreg, -n_downreg)
new_gene_sig <- right_join(sig_ids, new_gene_sig)

xx <- as.data.frame(org.Hs.eg.db::org.Hs.egSYMBOL) %>%
  setNames(., c("entrezgene", "gene_name"))

upreg_sig <- new_gene_sig %>% select(-down_genes_entrez) %>% separate_rows(up_genes_entrez, sep=";")
...
downreg_sig <- new_gene_sig %>% select(-up_genes_entrez) %>% separate_rows(down_genes_entrez, sep=";")
...
drg_tgts <- read_rds("data-raw/drugbank_db_metadata_v5.1.4.rds") %>% select(drugbank_id, targets) %>% distinct %>% na.omit
...
annots <- read_rds("data-raw/GSE92742_perts_in_DrugBank_03-30-21.rds")
